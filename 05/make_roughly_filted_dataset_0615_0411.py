...
if __name__ == '__main__':
    ...
    alpha, beta = get_xy_tick_size(bdry_coef, 0.75)
    ...
    uu.save_gpickle('grid_elements_750m_0615.pickle', {'grid_idx_list':start_end_idx, 'grid_list':masked_lat_lon1,  'alpha':alpha, 'beta':beta})


    new_temp_df.to_pickle(r'roughly_filtered_dataset_750m_0615.pkl')
    
    pd.DataFrame(data=[xx.wkt for xx in grid_list2]).to_csv(r'All_750m.csv')
    pd.DataFrame(data=[masked_lat_lon1[xx].wkt for xx in start_end_idx]).to_csv('Base_750m.csv')
