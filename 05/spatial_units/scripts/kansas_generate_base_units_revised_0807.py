...
temp_df = pd.read_pickle(r'roughly_filtered_dataset_0414.pkl')
temp_obj = uu.load_gpickle(r'grid_elements_0414.pickle')
...
def get_subgrid_dic1(grid_list, grid_idx_list, nsplit, alpha, beta):
    """fixed grid polygon들을 대상으로, subgrid 단위로 쪼갬"""
    subgrid_dic = {}
    for uni_temp_idx in grid_idx_list:
        subgrid_dic[uni_temp_idx] = split_grid_uni1(uni_grid=grid_list[uni_temp_idx],
                                                    x_size = alpha/nsplit, y_size=beta/nsplit)
    return subgrid_dic

###### 2)

def get_subgrid_idx(subgrid_polygons, usage_info,   target_cols=['start_grid','Start Location']):
    ...
    for uni_pnt in point_start1:
        for sub_idx, sub_poly in enumerate(subgrid_polygons[uni_pnt[0]]):
            if sub_poly.intersects(shapely.wkt.loads(uni_pnt[1])):
...
element1['subgrid_dic'] = get_subgrid_dic1(temp_obj['grid_list'], temp_obj['grid_idx_list'], float(4), temp_obj['alpha'], temp_obj['beta'])
...
element1['end_subgrid_idxs'] = get_subgrid_idx(element1['subgrid_dic'], temp_df, ['end_grid','End Location'])
...
element1['subgrid_avg_demand'] = ((((temp_df.groupby(['start_month','start_grid','start_basegrid_idx','start_subgrid_idx']).count()).loc[7])['year'])/31).unstack([-2,-1],fill_value=0).stack(-2).fillna(0)
...
zz1, zz2 = find_diag_grid_pair1(shp_dict, element3['pairwise_dist'])
len(zz1)

zz_dict0 = dict(zz2)

zz_dict = {}
for aa,bb in zz_dict0.items():
    if (zz_dict0[aa] == bb) & (zz_dict0[bb] == aa):
        if (aa not in zz_dict.keys()) & (aa not in zz_dict.values()):
            zz_dict[aa]=bb

for cc in zz_dict.items():
...
#vv1 = [set((aa,bb)) for aa,bb in zip(np.arange(28), pd.DataFrame.idxmin(element3['pairwise_dist'].replace(0,100)).values)]
...
def extract_temporal_repr(usage_info, main_masked_info):
    ...
    start_mask_h1 =usage_info.dropna().groupby(target_cols1).count().loc[7]['year'].unstack().fillna(0)
    end_mask_h1 =usage_info.dropna().groupby(target_cols2).count().loc[7]['year'].unstack().fillna(0)
...
def extract_demand_ratio(usage_info, main_masked_info):
    """Main Grid별 temporal representation 정보 산출"""
    target_cols1 = ['start_month','start_new_mask','start_hour']
    target_cols2 = ['end_month','end_new_mask','end_hour']
...
temp_df.loc[:, 'start_hour'] = pd.to_datetime(temp_df['Start Time']).dt.hour
temp_df.loc[:, 'end_hour'] = pd.to_datetime(temp_df['End Time']).dt.hour

element3['temporal_repr'] = extract_temporal_repr(temp_df, main_masked_dic)

demand_ratio = extract_demand_ratio(temp_df, main_masked_dic)
demand_ratio.loc[[0,40]].T.plot(xlabel='Hour', ylabel='Demand Ratio')
...
temp_df.final_new_mask.nunique()
...
uu.save_gpickle(r'kansas_generated_elements_0809.pickle',{'ele1':element1, 'ele2':element2, 'ele3':element3, 'shp_dict':shp_dict})
...
pd.DataFrame(data=[xx.wkt for xx in shp_dict['main_all_poly']]).to_csv(r'kansas_main_all_poly_0809.csv')


#pd.DataFrame(data=[xx.wkt for xx in shp_dict['main_support_poly']]).to_csv(r'kansas_main_support_poly.csv')

pd.DataFrame(data=[unary_union(xx).buffer(10**-14).wkt for xx in shp_dict['base_unit_poly_only_main']]).to_csv(r'kansas_base_unit_main_poly_0809.csv')
pd.DataFrame(data=[xx.wkt for xx in shp_dict['support_fixed_poly']]).to_csv(r'kansas_support_fixed_poly_0809.csv')


#%%

uu.save_gpickle(r'kansas_prop_45c_237f_0809.pickle', element3['base_unit_idx'])
temp_df.to_pickle(r'kansas_500m_4b_45c_dataset_0809.pkl')

#%%
"""
fixed_grid_dic = {}
for uni_idx, uni_fixed_grid in enumerate(temp_obj['grid_idx_list']):
    fixed_grid_dic[uni_idx] = [(uni_fixed_grid,0), (uni_fixed_grid,1), (uni_fixed_grid,2), (uni_fixed_grid,3)]
fixed_grid_dic
uu.save_gpickle(r'kansas_fixed_500m_262g_0507.pickle',fixed_grid_dic)
