...
from  kansas_main_indi_grids_prop_500m_train_vali_12345_0824 import LSTM1
...
def get_best_params(score_info, epoch_info):
    ...
    best_params = {'lr':hyper2[0], 'hidden': hyper2[1], 'seq_length':hyper2[1], 'batch':hyper1, 'epoch':hyper3}
    return best_params
...
if __name__ == '__main__':
    ...
    loss_dic_list = [uu.load_gpickle(r'kansas_prop_loss_history_vali_%s_%s.pickle'%(xx, save_date)) for xx in [1,2,3,4,5]]

    fixed_best_params = best_params_main(loss_dic_list,1,5)
    print(fixed_best_params)
    """ 
    trn: {'lr': 0.0007, 'hidden': 24, 'epoch': 33}
    vali: {'lr': 0.0003, 'hidden': 16,'epoch': 37}"""
