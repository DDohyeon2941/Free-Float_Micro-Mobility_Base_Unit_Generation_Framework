...
if __name__ == '__main__':
    ...
    loss_dic_list = [uu.load_gpickle(r'kansas_fixed_loss_history_vali_%s_%s.pickle'%(xx, save_date)) for xx in [1,2,3,4,5]]

    fixed_best_params = best_params_main(loss_dic_list,1,5)
    print(fixed_best_params)
    """ 
    trn: {'lr': 0.0007, 'hidden': 24, 'epoch': 17}
    vali: {'lr': 0.0007, 'hidden': 8, 'epoch': 24}"""
