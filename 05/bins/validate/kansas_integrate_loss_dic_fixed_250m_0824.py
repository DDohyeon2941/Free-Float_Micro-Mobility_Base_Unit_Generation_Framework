...
if __name__ == '__main__':
    ...
    loss_dic_list = [uu.load_gpickle(r'kansas_fixed_loss_history_vali_250m_%s_%s.pickle'%(xx, save_date)) for xx in [1,2,3,4,5]]

    fixed_best_params = best_params_main(loss_dic_list,1,5)
    print(fixed_best_params)
    """ 
    trn: {'lr': 0.0005, 'hidden': 24, 'epoch': 15}
    vali: {'lr': 0.0001, 'hidden': 24, 'epoch': 15}"""
