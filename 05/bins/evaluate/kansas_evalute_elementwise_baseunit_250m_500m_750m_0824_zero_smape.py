...
#%%
plt.plot(metric_2d_fixed[:,3][np.argsort(np.mean(all_real_y_fixed, axis=1))], label='250m')
plt.plot(metric_2d_fixed2[:,3][np.argsort(np.mean(all_real_y_fixed2, axis=1))], label='750m')
plt.plot(metric_2d_fixed3[:,3][np.argsort(np.mean(all_real_y_fixed3, axis=1))], label='500m')

plt.legend()
#%%
#plt.plot(np.sort(np.mean(all_real_y_fixed, axis=1)), label='250m')
plt.plot(np.sort(np.mean(all_real_y_fixed2, axis=1)), marker='x', label='750m')
plt.plot(np.sort(np.mean(all_real_y_fixed3, axis=1)),  marker='x', label='500m')
...
#%%
 ...
 (np.mean(all_real_y_fixed, axis=1) <= 1).sum() / all_real_y_fixed.shape[0]
 (np.mean(all_real_y_fixed2, axis=1) <= 1).sum() / all_real_y_fixed2.shape[0]
 (np.mean(all_real_y_fixed3, axis=1) <= 1).sum() / all_real_y_fixed3.shape[0]




#%%
((np.isnan(metric_2d_fixed).sum()/2)/ metric_2d_fixed.shape[0]).round(2)
((np.isnan(metric_2d_fixed2).sum()/2)/ metric_2d_fixed2.shape[0]).round(2)
((np.isnan(metric_2d_fixed3).sum()/2)/ metric_2d_fixed3.shape[0]).round(2)
...
score_df.groupby(['m','weight']).mean().unstack(0).T.to_csv(r'kansas_fixed_250m_500m_750m_uniform_weighted_0824.csv')
