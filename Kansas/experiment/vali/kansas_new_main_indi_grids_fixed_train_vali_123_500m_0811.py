...
if __name__ == '__main__':
    ...
    for uni_try in range(try_num):
        print('start %s try'%(uni_try))
        vali_num = 1
        temp_pkl = uu.load_gpickle(r'kansas_fixed_train_valid_12345_scaler_dataset_%s.pickle'%(save_date))[vali_num]
        ...
        for uni_lr, uni_hidden, uni_batch in itertools.product(lr_range, hidden_range, batch_size):
    
            aa1=main(train_df, vali_df, uni_lr, uni_hidden, seq_length, uni_batch)
            loss_dic[(uni_lr, uni_hidden, uni_batch)] = aa1
            print('%s / %s' % (exp_round, len(lr_range)*len(hidden_range)*len(batch_size)))
            exp_round+=1
            aa1=None
        ...
        uu.save_gpickle(r'kansas_fixed_loss_history_vali_500m_%s_%s_%s.pickle'%(uni_try, vali_num, save_date1), loss_dic)
