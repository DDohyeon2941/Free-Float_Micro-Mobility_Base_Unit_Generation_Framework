...
if __name__ == '__main__':
    ...
    uu.save_gpickle('grid_elements_250m_0615.pickle', {'grid_idx_list':start_end_idx, 'grid_list':masked_lat_lon1,  'alpha':alpha, 'beta':beta})


    new_temp_df.to_pickle(r'roughly_filtered_dataset_250m_0615.pkl')
    
    pd.DataFrame(data=[xx.wkt for xx in grid_list2]).to_csv(r'All_250m.csv')
    pd.DataFrame(data=[masked_lat_lon1[xx].wkt for xx in start_end_idx]).to_csv('Base_250m.csv')
