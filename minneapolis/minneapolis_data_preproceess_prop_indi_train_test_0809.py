...
def get_demand_df(all_info, is_train=True):
    ...
    SEL_COLS = ['date','start_hour','final_new_mask']
    ...
    new_demand_df = pd.DataFrame(index=demand_df.index, columns=np.arange(NUM_BASE_UNIT),
                                 data=0)
    new_demand_df.loc[demand_df.index, demand_df.columns] = demand_df.values
...
if __name__ == '__main__':
    ...
    uu.save_gpickle(r'minneapolis_prop_train_test_scaler_dataset_0809.pickle', {'scaler':scaler, 'train_dataset':train_df, 'test_dataset':test_df})
