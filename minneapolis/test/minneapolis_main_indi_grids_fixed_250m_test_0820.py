...
from minneapolis_main_indi_grids_fixed_750m_train_vali_12345_0820 import SharingBikes, LSTM1, train, validate, get_smape
...
if __name__ == '__main__':
    ...
    save_date1 = '0820'
    best_params = {'lr':0.0007, 'hidden': 16, 'seq_length':8, 'batch':8, 'epoch':14}
    ...
    uu.save_gpickle(r'minneapolis_fixed_model_loss_test_250m_%s.pickle'%(save_date1), aa2)
    #aa2 = uu.load_gpickle(r'minneapolis_fixed_model_loss_test_250m_%s.pickle'%(save_date))
    #%%
    ...
    #%%
    sstep_num = int((np.array(grid_num_li).shape[0]/new_train_df['level_2'].nunique()))
    yy_li = []
    pp_li = []
    sstep_li = []
    ...
    uu.save_gpickle(r'minneapolis_real_pred_y_fixed_250m_%s.pickle'%(save_date1),{'pred_y':np.array(pp_li),'real_y':np.array(yy_li)})
    ...
    o_score_df.to_pickle(r'minneapolis_o_score_df_fixed_250m_%s.pkl'%(save_date1))
