...
if __name__ == '__main__':
    ...
    save_date1 = '0820'
    best_params = {'lr':0.0003, 'hidden': 16, 'seq_length':8, 'batch':8, 'epoch':20}
    ...
    uu.save_gpickle(r'minneapolis_fixed_model_loss_test_%s.pickle'%(save_date1), aa2)
    #aa2 = uu.load_gpickle(r'minneapolis_fixed_model_loss_test_%s.pickle'%(save_date))
    #%%
    ...
    uu.save_gpickle(r'minneapolis_real_pred_y_fixed_%s.pickle'%(save_date1),{'pred_y':np.array(pp_li),'real_y':np.array(yy_li)})
    ...
    o_score_df.to_pickle(r'minneapolis_o_score_df_fixed_%s.pkl'%(save_date1))
