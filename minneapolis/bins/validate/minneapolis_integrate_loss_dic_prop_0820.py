...
from  minneapolis_main_indi_grids_fixed_750m_train_vali_12345_0820 import LSTM1
import pandas as pd
import numpy as np
...
if __name__ == '__main__':
    ...
    loss_dic_list = [uu.load_gpickle(r'minneapolis_prop_loss_history_vali_%s_%s.pickle'%(xx, save_date)) for xx in [1,2,3,4,5]]

    fixed_best_params = best_params_main(loss_dic_list,1,5)
    print(fixed_best_params)
    """ 
    trn: {'lr': 0.0009, 'hidden': 24, 'epoch': 18}
     vali: {'lr': 0.0001, 'hidden': 24, 'epoch': 24}"""

    #%%
    gg1= merge_loss_valis(loss_dic_list, 1,val_num=5)
